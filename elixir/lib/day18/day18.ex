defmodule Aoc2023.Day18 do
  ...
  def part2(part \\ :ex2) do
    etl_input(part)
    |> Enum.map(fn [_, _, amount, d] -> [@dir_2[d], Integer.parse(amount, 16) |> then(fn {a, _} -> a end)] end)
    |> compute_internal_points()
  end
  ...
  def compute_internal_points(list) do
    {boundary, length} = Enum.reduce(list, {[[0, 0]], 0}, fn [d, amount], acc ->
      {points, length} = acc
      {points ++ [(multiply(@dir[d], amount)
        |> Enum.zip(List.last(points)) |> Enum.map(&Tuple.sum/1))], length + amount}
      end)
